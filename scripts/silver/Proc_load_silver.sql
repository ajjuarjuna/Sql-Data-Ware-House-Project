	...
	BEGIN TRY
		...
		SELECT
			...
			CASE WHEN sls_sales <= 0 OR sls_sales IS NULL OR sls_sales != sls_quantity * ABS(sls_price)
				 ...
				 ELSE sls_sales
			END AS sls_sales,       -- Recalculate Sales if original is missing or incorrect
			sls_quantity,
		...
		gen)
		SELECT
		CASE WHEN cid LIKE 'NAS%'THEN SUBSTRING(cid,4,LEN(cid))      -- Remove prefix 'NAS' where it is present
			 ELSE cid        
		END AS cid,
		CASE WHEN bdate > GETDATE() THEN NULL                        -- Convert future birthdates to Null
			 ELSE bdate
		END AS bdate,
		CASE WHEN UPPER(TRIM(gen)) IN ('M','MALE') THEN 'Male'       -- Normalize gender Values and handle Unknown Values
